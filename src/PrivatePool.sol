...
contract PrivatePool is ERC721TokenReceiver {
        ...
        )
    {
        ...
        uint256 weightSum = sumWeightsAndValidateProof(
            tokenIds,
            tokenWeights,
            proof
        );
        ...
        virtualBaseTokenReserves += uint128(
            netInputAmount - feeAmount - protocolFeeAmount
        );
        ...
        if (baseToken != address(0)) {
            ...
            // if the protocol fee is set then pay the protocol fee
            if (protocolFeeAmount > 0)
                factory.safeTransferETH(protocolFeeAmount);
        ...
        )
    {
        ...
        if (useStolenNftOracle) {
            IStolenNftOracle(stolenNftOracle).validateTokensAreNotStolen(
                nft,
                tokenIds,
                stolenNftProofs
            );
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (payRoyalties) {
                // calculate the sale price (assume it's the same for each NFT even if weights differ)
                uint256 salePrice = (netOutputAmount +
                    feeAmount +
                    protocolFeeAmount) / tokenIds.length;
    ...
    function change(
    ...
    ) external payable returns (uint256 feeAmount, uint256 protocolFeeAmount) {
        // ~~~ Checks ~~~ //
        ...
        // fix stack too deep
        {
            ...
            uint256 inputWeightSum = sumWeightsAndValidateProof(
                inputTokenIds,
                inputTokenWeights,
                inputProof
            );
            ...
            uint256 outputWeightSum = sumWeightsAndValidateProof(
                outputTokenIds,
                outputTokenWeights,
                outputProof
            );
    ...
    function setUseStolenNftOracle(
    ...
    function sumWeightsAndValidateProof(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            leafs[i] = keccak256(
                bytes.concat(
                    keccak256(abi.encode(tokenIds[i], tokenWeights[i]))
                )
            );
        ...
        )
    {
        ...
        uint256 inputAmount = FixedPointMathLib.mulDivUp(
            ...
            (virtualNftReserves - outputAmount)
        );

        protocolFeeAmount =
            (inputAmount * Factory(factory).protocolFeeRate()) /
            10_000;
        ...
        )
    {
        ...
        uint256 outputAmount = (inputAmount * virtualBaseTokenReserves) /
            (virtualNftReserves + inputAmount);

        protocolFeeAmount =
            (outputAmount * Factory(factory).protocolFeeRate()) /
            10_000;
