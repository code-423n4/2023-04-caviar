...
contract PrivatePoolMetadata {
    ...
    function attributes(uint256 tokenId) public view returns (string memory) {
        PrivatePool privatePool = PrivatePool(
            payable(address(uint160(tokenId)))
        );
        ...
        bytes memory _attributes = abi.encodePacked(
            ...
            ",",
            trait(
                "Virtual base token reserves",
                Strings.toString(privatePool.virtualBaseTokenReserves())
            ),
            ",",
            trait(
                "Virtual NFT reserves",
                Strings.toString(privatePool.virtualNftReserves())
            ),
            ",",
            ...
            trait(
                "NFT balance",
                Strings.toString(
                    ERC721(privatePool.nft()).balanceOf(address(privatePool))
                )
            ),
            ",",
            trait(
                "Base token balance",
                Strings.toString(
                    privatePool.baseToken() == address(0)
                        ? address(privatePool).balance
                        : ERC20(privatePool.baseToken()).balanceOf(
                            address(privatePool)
                        )
                )
            )
    ...
    function svg(uint256 tokenId) public view returns (bytes memory) {
        ...
        bytes memory _svg;
        {
            ...
            _svg = abi.encodePacked(
                ...
                "Private pool: ",
                Strings.toHexString(address(privatePool)),
                "</text>",
                '<text x="24px" y="72px" font-size="12">',
                "Base token: ",
                Strings.toHexString(privatePool.baseToken()),
                "</text>",
                '<text x="24px" y="96px" font-size="12">',
                "NFT: ",
                Strings.toHexString(privatePool.nft()),
                "</text>"
            );
        }

        {
            ...
            _svg = abi.encodePacked(
                ...
                "Fee rate (bps): ",
                Strings.toString(privatePool.feeRate()),
                "</text>"
            );
        }
